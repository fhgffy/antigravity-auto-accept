...
3. **The Hacker (user32.dll Keyboard Injection + RuntimeId Cache)**
   ...
   ***v1.3.6 The Perfect Silence Update:*** Fixed a critical bug where the AutoClicker would act like a "wild dog" and infinitely loop-click historical `Expand` buttons in the chat log, causing severe screen jitter. Also added a strict Foreground Window check: the script will **never** steal your system focus if you are actively typing in another app (like Chrome) when a prompt appears in the background.
   > ***v1.3.8 Ultimate Anti-Jitter Upgrade:*** Completely eradicated the scrollbar twitching issue caused by the presence of multiple historical `Always run` buttons in the chat history. The script now aggressively ignores disabled/off-screen prompts and enforces a strict `One-Click-Per-Cycle` break mechanism to prevent fighting over focus.
   > ***v1.3.9 Browser JS Execution Fix (2026-03-21):*** Added support for auto-accepting Antigravity's JavaScript Execution policy dialogs. The `Proceed` button is now matched and auto-clicked when the agent requests to execute JavaScript in the browser. Also added `Execute`, `继续`, `执行` keywords (Issue #1).
   > ***v1.3.9 浏览器 JS 执行修复 (2026-03-21)：*** 新增对 Antigravity 浏览器 JavaScript 执行权限对话框的自动确认支持。Agent 请求在浏览器中执行 JS 时，`Proceed`（继续）按钮会被自动点击。同时新增 `Execute`、`继续`、`执行` 关键词（Issue #1）。
   > ***v1.4.0 Directory Permission Auto-Accept (2026-03-21):*** Fixed a critical bug where `Allow Once` and `Allow This Conversation` buttons in Antigravity's directory access permission dialogs were NOT being auto-clicked. The root cause was that the regex `^allow$` only matched the exact word "Allow", but these buttons contain multi-word text like "Allow This Conversation". Now matches `Allow Once`, `Allow This Conversation`, `Allow All`, and their Chinese equivalents `允许本次`, `允许此对话`, `全部允许`.
   > ***v1.4.0 目录权限自动放行 (2026-03-21)：*** 修复了一个关键 bug —— Antigravity 的目录访问权限对话框中的 `Allow Once`（允许本次）和 `Allow This Conversation`（允许此对话）按钮无法被自动点击。根因是正则 `^allow$` 只能精确匹配单词 "Allow"，无法命中多词按钮。现已新增 `Allow Once`、`Allow This Conversation`、`Allow All` 及中文对应词的匹配。
